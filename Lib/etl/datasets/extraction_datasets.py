...
class ExtractConferenceDataSet(ExtractionDataSet):
    ...
    def extract(self, cfbd_client, db_client) -> bool:
        ...
        try:
            conf_coll = db_client.get_cfb_collection(
            ...
            for conf in conferences:
                ...
                count += 1
            log.debug(f"ExtractConferenceDataSet: Extracted {count} entities")
        except Exception as e:
            log.exception(
                f"ExtractConferenceDataSet: Exception when writing to DB: {e}"
            )
            return False
...
class ExtractGamesDataSet(ExtractionDataSet):
    ...
    def __init__(
        self, year_list: list[int], class_list: list[str], week_list: list[int] = None
    ):
        super().__init__()
    ...
    def extract(self, cfbd_client, db_client) -> bool:
        ...
        for year in self.year_list:
            ...
            try:
                games_coll = db_client.get_cfb_collection(
        ...
        log.debug(f"ExtractGamesDataSet: Extracted {count} games")
        return True
...
class ExtractTeamDataSet(ExtractionDataSet):
    ...
    def extract(self, cfbd_client: CfbdConnection, db_client: DbConnection) -> bool:
        ...
        if self.class_list == None or len(self.class_list) == 0:
            log.warning(
                "ExtractTeamDataSet: Skipping since no classifications were specified"
            )
            return True
        ...
        for year in self.year_list:
            ...
            try:
                team_coll = db_client.get_cfb_collection(
                ...
                for team in teams:
                    ...
                    count += 1
                log.debug(f"ExtractTeamDataSet: Extracted {count} entities")
            except Exception as e:
...
class ExtractVenueDataSet(ExtractionDataSet):
    ...
    def extract(self, cfbd_client, db_client) -> bool:
        ...
        try:
            venue_coll = db_client.get_cfb_collection(
            ...
            for venue in venues:
                ...
                count += 1
            log.debug(f"ExtractVenueDataSet: Extracted {count} entities")
        except Exception as e:
...
class ExtractGameTeamStats(ExtractionDataSet):
    ...
    def __init__(self, year_list: list[int], week_list: list[int] = None):
        super().__init__()
        self.year_list = year_list
        self.week_list = list(range(1, 17)) if week_list is None else week_list

    def extract(self, cfbd_client, db_client) -> bool:
        if self.year_list is None or len(self.year_list) == 0:
            log.warning("ExtractGameTeamStats: Skipping since no years were specified")
            return True

        if self.week_list is None or len(self.week_list) == 0:
            log.warning("ExtractGameTeamStats: Skipping since no weeks were specified")
            return True
        ...
        for year in self.year_list:
            for week in self.week_list:
                ...
                game_stats = api_call(
                    lambda: api.get_game_team_stats(year=year, week=week)
                )

                if game_stats is None or len(game_stats) == 0:
                    log.warning(
                        f"ExtractGameTeamStats: No game stats data for year {year}, week {week}"
                    )
                    continue

                try:
                    stats_coll = db_client.get_cfb_collection(
                        Databases.extraction, ExtractionCollections.game_team_stats
                    )
                    for stat in game_stats:
                        stat_dict = stat.to_dict()
                        query = {"id": stat.id}
                        stats_coll.replace_one(query, stat_dict, upsert=True)
                        count += 1
                except Exception as e:
                    log.exception(
                        f"ExtractGameTeamStats: Exception when writing to DB: {e}"
                    )
                    return False

        log.debug(f"ExtractGameTeamStats: Extracted {count} entities")
        return True

    def cleanup(self, db_client):
        try:
            cleanup_extraction_collections(
                db_client, ExtractionCollections.game_team_stats
            )
        except Exception as e:
            log.exception(f"ExtractGameTeamStats: Exception when cleaning up: {e}")
