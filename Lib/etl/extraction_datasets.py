...
class ExtractTeamDataSet(ExtractionDataSet):
    ...
    def __init__(self, log: logging.Logger, year_list: list[int], class_list: list[str]):
        super().__init__(log)
    ...
    def extract(self, cfbd_client: CfbdConnection, db_client: DbConnection) -> bool:
        if self.year_list == None or len(self.year_list) == 0:
            self.log.warning("ExtractTeamDataSet: Skipping since no years were specified")
            return True
        if self.class_list == None or len(self.class_list) == 0:
            self.log.warning("ExtractTeamDataSet: Skipping since no classifications were specified")
            return True
        ...
        for year in self.year_list:
            ...
            if teams == None:
                self.log.warning("ExtractTeamDataSet: No data fetched from API")
                return True
            ...
            try:
                ...
                for team in teams:
                    ...
                    count += 1
                self.log.info(f"ExtractTeamDataSet: Extracted {count} entities")
            except Exception as e:
                self.log.exception(f"ExtractTeamDataSet: Exception when writing to DB: {e}")
                return False
...
class ExtractConferenceDataSet(ExtractionDataSet):
    ...
    def extract(self, cfbd_client, db_client) -> bool:  
        ...
        if conferences == None:
            self.log.warning("ExtractConferenceDataSet: No data fetched from API")
            return True
        ...
        try:
            ...
            for conf in conferences:
                ...
                count += 1
            self.log.info(f"ExtractConferenceDataSet: Extracted {count} entities")
        except Exception as e:
            self.log.exception(f"ExtractConferenceDataSet: Exception when writing to DB: {e}")
            return False
    ...
    def cleanup(self, db_client):
        try:
            ...
            self.log.exception(f"ExtractConferenceDataSet: Exception when cleaning up: {e}")
...
class ExtractVenueDataSet(ExtractionDataSet):
    ...
    def extract(self, cfbd_client, db_client) -> bool:
        ...
        if venues == None:
            self.log.warning("ExtractVenueDataSet: No data fetched from API")
            return True
        ...
        try:
            ...
            for venue in venues:
                ...
                count += 1
            self.log.info(f"ExtractVenueDataSet: Extracted {count} entities")
        except Exception as e:
            self.log.exception(f"ExtractVenueDataSet: Exception when writing to DB: {e}")
            return False
    ...
    def cleanup(self, db_client):
        try:
            ...
            self.log.exception(f"ExtractVenueDataSet: Exception when cleaning up: {e}")

class ExtractGamesDataSet(ExtractionDataSet):
    """Extract games from CFBD."""

    def __init__(self, log, year_list: list[int], class_list: list[str], week_list: list[int] = None):
        super().__init__(log)
        self.year_list = year_list
        self.class_list = class_list
    ...
    def extract(self, cfbd_client, db_client) -> bool:
        ...
        for year in self.year_list:
            ...
            if games is None or len(games) == 0:
                self.log.warning(f"ExtractGamesDataSet: No games data for year {year}")
                continue
            ...
            try:
                ...
                self.log.exception(f"ExtractGamesDataSet: Exception when writing to DB: {e}")
                return False
        
        self.log.info(f"ExtractGamesDataSet: Extracted {count} games")
        return True
    ...
    def cleanup(self, db_client):
        try:
            ...
            self.log.exception(f"ExtractGamesDataSet: Exception when cleaning up: {e}")